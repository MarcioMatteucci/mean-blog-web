...
module.exports = (router) => {
  ...
  router.post('/register', (req, res) => {
    ...
    if (!req.body.email) {
      res.json({ success: false, message: 'Debe ingresar un E-mail' }); // Return error
    } else {
      // Check if username was provided
      if (!req.body.username) {
        res.json({ success: false, message: 'Debe ingresar un Nombre de Usuario' }); // Return error
      } else {
        // Check if password was provided
        if (!req.body.password) {
          res.json({ success: false, message: 'Debe ingresar una Contraseña' }); // Return error
        } else {
          ...
          user.save((err) => {
            ...
            if (err) {
              ...
              if (err.code === 11000) {
                res.json({ success: false, message: 'Nombre de Usuario y/o E-mail ya existen' }); // Return error
              } else {
                ...
                if (err.errors) {
                  ...
                  res.json({ success: false, message: 'No se pudo registrar el Usuario. Error: ', err }); // Return error if not related to validation
                }
              }
            } else {
              res.json({ success: true, message: 'Usuario Registrado!' }); // Return success
            }
          });
        }
      }
